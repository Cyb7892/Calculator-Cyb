...
class Calculator:
    ...
    def on_button_click(self, text):
        ...
        elif text in ['+', '-', '×', '÷', '^']:  # 增加对^的支持
            # 运算符
            # 如果正在输入分数，先计算分数值
            if self.is_fraction and self.numerator and self.denominator:
                self.calculate_fraction()
                
            # 允许在表达式结尾是运算符或左括号时添加运算符
            if self.current_input or self.expression.endswith(tuple(['(', '[', '{', '+', '-', '×', '÷', '^'])):
    ...
    def handle_bracket_input(self, bracket):
        ...
        # 右括号需要检查是否有匹配的左括号
            ...
            # 确保有对应的左括号，并且当前输入不为空或表达式以左括号结尾
            if (self.bracket_counts[matching_left] > self.bracket_counts[bracket] and 
                (self.current_input or self.expression.endswith(matching_left))):
